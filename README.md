...
WordPress オリジナルテーマ（親テーマなし・フルスクラッチ）。

建築スタジオ向けのコーポレートサイトを想定した、**スキル確認用のソース**です。完成品の納品一式ではなく、ゼロからテーマを設計できることがコード上で分かる粒度にしています。

| 項目 | 内容 |
| --- | --- |
| 担当 | 情報設計 / テーマ実装 / フロント（HTML・CSS・JS） / SEO 内部 |
| 使用技術 | PHP 8.1+ / WordPress 6.4+ / ACF Pro（未導入時は native meta） / Polylang・WPML 対応 / Google Maps JavaScript API / WP All Import / WP-CLI |
| 想定期間 | 骨格〜主要テンプレートで約 1〜2 週間相当 |

スクリーンショット（`screenshot.png`）がトップの見た目です。
...
- **Google Maps**: `inc/maps.php` + `assets/js/maps.js`（交差観察後に API を読む。キーはカスタマイザーのみ）
...
```
inc/                 関心ごとの PHP（巨大な functions.php にしない）
...
会員機能、EC、ブロックテーマ化、本本番の画像書き出し、自動テストです。選考でコードの書き方と情報設計を見てもらうための範囲に止めています。
